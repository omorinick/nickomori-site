...
const timingColorMap: Record<TimingColor, { badge: string; slot: string }> = {
  daily:   { badge: 'bg-secondary text-muted-foreground',    slot: 'text-[10px] font-mono text-muted-foreground uppercase tracking-wider' },
  monday:  { badge: 'bg-warning-subtle text-warning',        slot: 'text-[10px] font-mono text-warning uppercase tracking-wider' },
  friday:  { badge: 'bg-success-subtle text-success',        slot: 'text-[10px] font-mono text-success uppercase tracking-wider' },
  monthly: { badge: 'bg-info-subtle text-info',              slot: 'text-[10px] font-mono text-info uppercase tracking-wider' },
}

const cadenceSlots = [
  {
    timeLabel: 'MON AM',
    color: 'monday' as TimingColor,
    automations: ['Alignment Scan', 'Daily Context Brief'],
  },
  {
    timeLabel: 'FRI AM',
    color: 'friday' as TimingColor,
...
For a presentation header:
  Font:  Graphik Black — not Bold (Black is the correct weight for display use)
...
function ToolPills({ tools }: { tools: string[] }) {
  ...
  return (
    <div className="flex flex-wrap gap-1.5 mt-3">
      {tools.map((tool) => (
        <span key={tool} className="bg-secondary text-muted-foreground px-2 py-0.5 rounded text-xs">
          {tool}
        </span>
      ))}
    </div>
...
function AutomationCard({
  ...
  const colors = timingColorMap[automation.timingColor]
  ...
  return (
    <div
      ...
      className={[
        'rounded-xl border px-5 py-5 cursor-pointer select-none',
...
function SkillCard({ skill }: { skill: (typeof skills)[0] }) {
  ...
  return (
    <div
      ...
      onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') setOpen(!open) }}
      className={[
        'rounded-xl border px-5 py-5 cursor-pointer select-none flex flex-col',
        'transition-all duration-150',
        open
          ? 'border-border-hover bg-card'
          : 'border-border bg-card hover:border-border-hover hover:-translate-y-0.5',
        ...
        <section className="mb-12">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-2 mb-8">
            {cadenceSlots.map((slot) => {
              const isActive = activeSlot === slot.timeLabel
              const colors = timingColorMap[slot.color]
              return (
                <div
                  key={slot.timeLabel}
                  role="button"
                  tabIndex={0}
                  onClick={() => setActiveSlot(isActive ? null : slot.timeLabel)}
                  onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') setActiveSlot(isActive ? null : slot.timeLabel) }}
                  className={[
                    ...
                    isActive
                      ? 'border-border-hover bg-card'
                      : 'border-border bg-card hover:border-border-hover hover:-translate-y-0.5',
                    activeSlot && !isActive ? 'opacity-40' : '',
                  ].join(' ')}
                >
                  <p className={`${colors.slot} mb-1.5`}>{slot.timeLabel}</p>
          ...
          <div className="flex flex-col gap-4">
            {automations.map((automation) => (
              <AutomationCard
                ...
                dimmed={activeSlot !== null && automation.slot !== activeSlot}
              />
            ))}
          </div>
        </section>
