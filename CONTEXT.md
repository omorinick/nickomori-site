...
- `substack-case` is the second deck: a full PM interview case study (PayPal Merchant Trajectory System) with a vertical spine, an expandable full-page "deep dive" sub-presentation, seven modal-viewable sanitized artifact recreations, a three-stage progressive research-pivot reveal, evidence-status review flags and presenter notes behind a `REVIEW_TOOLS` constant (flip to `false` before presenting to hide all review chrome), and a minimal transparent site header on `/case-studies/*` routes. Reconstruction provenance is tracked in the project memory ledger, not the repo.
- The Vault landing page (`/vault`, behind the vault gate) is now a two-collection hub: Personal artifacts (vault passphrase) and a registry-driven Case Studies list (each deck keeps its own per-slug passphrase) — this is the only navigation to case studies; they are deliberately absent from public nav
